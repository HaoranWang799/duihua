...
const sendJson = (res: ServerResponse, statusCode: number, payload: object) => {
...
const getAudioContentType = (format: string) => {
  switch (format) {
    case 'wav':
      return 'audio/wav'
    case 'ogg':
...
const getSystemConfig = (mode: string): AppSystemConfig => {
...
const fishAudioProxyMiddleware = async (
  req: IncomingMessage,
  res: ServerResponse,
  systemConfig: FishAudioSystemConfig,
) => {
  if (req.method === 'GET') {
    sendJson(res, 200, {
...
const storyGenerationMiddleware = async (
  ...
  try {
    const rawBody = await readBody(req)
    const { keywords } = JSON.parse(rawBody) as { keywords?: string }
    ...
    sendJson(res, 200, JSON.parse(outputText) as object)
  } catch (error) {
    sendJson(res, 500, {
      message:
...
const appProxyPlugin = (systemConfig: AppSystemConfig) => ({
...
export default defineConfig(({ mode }) => {
  const systemConfig = getSystemConfig(mode)

  return {
    plugins: [react(), appProxyPlugin(systemConfig)],
  }
})
